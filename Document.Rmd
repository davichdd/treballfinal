...
d2 %>% ggplot(aes(x = afr, y = logit_VOX))+
  geom_point()
```
###Anàlisi bivariat Renta neta persona
...
```{r}
model1 <- lm(logit(VOX) ~ Renta_neta_persona, data=d2)
summary(model1)
```
```{r}
names(d2)
...
```{r}
model2 <- lm(logit(VOX) ~ Renta_neta_persona + esp + atur_m24 + pob_t + UE, data=d2)
summary(model2)
```
```{r}
model3 <- lm(logit(VOX) ~ Renta_neta_persona + esp + atur_m24 + pob_t + UE + atur_d + atur_h, data=d2)
summary(model3)
```
```{r}
model4 <- lm(logit(VOX) ~ Renta_neta_persona + esp + pob_t + UE + atur, data=d2)
summary(model4)
```
```{r}
model5 <- lm(logit(VOX) ~ Renta_neta_persona + esp + atur_m24 + pob_t + UE + atur_d + atur_h + asia, data=d2)
summary(model5)
```
```{r}
model6 <- lm(logit(VOX) ~ Renta_neta_persona + esp + atur_m24 + pob_t + UE + atur_d + atur_h + ame, data=d2)
summary(model6)
```
```{r}
model7 <- lm(logit(VOX) ~ Renta_neta_persona + esp + atur_m24 + pob_t + UE + atur_d + atur_h + ame + oce_apa, data=d2)
summary(model7)
```
```{r}
model8 <- lm(logit(VOX) ~ Renta_neta_persona + esp + atur_m24 + pob_t + UE + atur_d + atur_h + ame + oce_apa + afr, data=d2)
summary(model8)
```
```{r}
names(d2)
```
```{r}
model9 <- lm(logit(VOX) ~ Renta_neta_persona + esp + atur_m24 + pob_t + UE + atur_d + atur_h + ame + oce_apa + afr + e40_49 + e50_59 + m19, data=d2)
summary(model9)
```
